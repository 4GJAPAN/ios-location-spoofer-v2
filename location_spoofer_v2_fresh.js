...
(function () {
  ...
  function canUsePersistentStore() {
    return typeof $persistentStore !== "undefined" && $persistentStore && $persistentStore.read && $persistentStore.write;
  }

  function readStoreJson(key) {
  ...
  function isStoredConfigCandidate(value) {
  ...
  function readBestPersistentConfig() {
    var current = readStoreJson(STORE_KEYS.current);
    if (isStoredConfigCandidate(current)) {
      return current;
    }
    var lastGood = readStoreJson(STORE_KEYS.lastGood);
    if (isStoredConfigCandidate(lastGood)) {
      return lastGood;
    }
    return null;
  }

  function persistLastGoodConfig(config) {
    if (!canUsePersistentStore()) {
      return;
    }
    var clean = clonePlainObject(config);
    delete clean._effectiveLatitude;
    delete clean._effectiveLongitude;
    delete clean._loadedFromPersistentStore;
    writeStoreJson(STORE_KEYS.lastGood, clean);
  }

  function hashSeed(value) {
    var text = String(value == null ? "2772" : value);
    var hash = 2166136261;
  ...
  function applyStableJitter(config) {
    config._effectiveLatitude = config.latitude;
    config._effectiveLongitude = config.longitude;
    if (!(config.jitterRadius > 0)) {
      return config;
    }

    var interval = Math.max(10000, config.jitterIntervalMs || 30000);
    var bucket = Math.floor(Date.now() / interval);
    var seed = hashSeed(String(config.sessionSeed) + ":" + String(bucket));
    var angle = seededUnit(seed) * Math.PI * 2;
    var distance = Math.sqrt(seededUnit(seed ^ 0x9e3779b9)) * config.jitterRadius;
    var north = Math.cos(angle) * distance;
    var east = Math.sin(angle) * distance;
    var earthRadius = 6378137;
    var latRadians = config.latitude * Math.PI / 180;

    config._effectiveLatitude = config.latitude + (north / earthRadius) * 180 / Math.PI;
    var cosLat = Math.cos(latRadians);
    if (Math.abs(cosLat) > 0.000001) {
      config._effectiveLongitude = config.longitude + (east / (earthRadius * cosLat)) * 180 / Math.PI;
    }
    return config;
  }

  function runtimeState() {
    return readStoreJson(STORE_KEYS.runtime) || {
      patchCount: 0,
      errorCount: 0,
      consecutiveErrors: 0,
      lastPatchAt: 0,
  ...
  function recordRuntimeSuccess(config, wifiCount, cellCount) {
    if (!canUsePersistentStore()) {
      return;
    }
    var state = runtimeState();
    var now = Date.now();
    state.patchCount = Math.max(0, Number(state.patchCount) || 0) + 1;
    state.consecutiveErrors = 0;
    state.lastError = "";
    state.lastPatchAt = now;
    state.lastHost = typeof $request !== "undefined" && $request && $request.url ? String($request.url).split("/")[2] || "" : "";
    state.lastLatitude = config.latitude;
    state.lastLongitude = config.longitude;
    state.lastWifiCount = wifiCount || 0;
    state.lastCellCount = cellCount || 0;
    writeStoreJson(STORE_KEYS.runtime, state);
  }
  ...
  function recordRuntimeError(error) {
    ...
    var state = runtimeState();
    state.errorCount = Math.max(0, Number(state.errorCount) || 0) + 1;
    state.consecutiveErrors = Math.max(0, Number(state.consecutiveErrors) || 0) + 1;
    state.lastErrorAt = Date.now();
    state.lastError = String(error && error.message ? error.message : error || "unknown error").slice(0, 500);
    writeStoreJson(STORE_KEYS.runtime, state);
  }
  ...
  function normalizeConfig(input) {
    ...
    cfg.resetOnError = parseBoolean(cfg.resetOnError, false);
    cfg.resetOnNetworkChange = parseBoolean(cfg.resetOnNetworkChange, false);
    ...
    cfg.jitterRadius = Number(cfg.jitterRadius || 0);
    cfg.jitterIntervalMs = Math.trunc(Number(cfg.jitterIntervalMs || 30000));
    cfg.sessionSeed = Math.trunc(Number(cfg.sessionSeed || 2772));
    cfg.expiresAt = Math.trunc(Number(cfg.expiresAt || 0));
    ...
    if (!Number.isFinite(cfg.horizontalAccuracy) || cfg.horizontalAccuracy < 1) {
      cfg.horizontalAccuracy = DEFAULT_CONFIG.horizontalAccuracy;
    }
    if (!Number.isFinite(cfg.verticalAccuracy) || cfg.verticalAccuracy < 1) {
      cfg.verticalAccuracy = DEFAULT_CONFIG.verticalAccuracy;
    }
    if (!Number.isFinite(cfg.altitude)) {
      cfg.altitude = DEFAULT_CONFIG.altitude;
    }
    if (!Number.isFinite(cfg.jitterRadius) || cfg.jitterRadius < 0 || cfg.jitterRadius > 1000) {
      cfg.jitterRadius = 0;
    }
    if (!Number.isFinite(cfg.jitterIntervalMs) || cfg.jitterIntervalMs < 10000) {
      cfg.jitterIntervalMs = 30000;
    }
    ...
    if (cfg.expiresAt > 0 && Date.now() >= cfg.expiresAt) {
      cfg.enabled = false;
    }
    return applyStableJitter(cfg);
  }

  function patchLocation(locationPayload, config) {
    var parts = [];
    var fields = locationPayload.length ? parseFields(locationPayload) : [];
    var preserveAltitude = config.altitudeMode === "preserve";
    var preserveMotion = config.motionMode === "preserve";

    for (var i = 0; i < fields.length; i += 1) {
      var fieldNumber = fields[i].fieldNumber;
      var alwaysReplaced = fieldNumber === 1 || fieldNumber === 2 || fieldNumber === 3 || fieldNumber === 4;
      var altitudeReplaced = !preserveAltitude && (fieldNumber === 5 || fieldNumber === 6);
      var motionReplaced = !preserveMotion && (fieldNumber === 11 || fieldNumber === 12);
      if (!alwaysReplaced && !altitudeReplaced && !motionReplaced) {
        parts.push(fields[i].raw);
      }
    }

    parts.push(makeVarintField(1, coordToInt(config._effectiveLatitude != null ? config._effectiveLatitude : config.latitude)));
    parts.push(makeVarintField(2, coordToInt(config._effectiveLongitude != null ? config._effectiveLongitude : config.longitude)));
  ...
  function loadRuntimeConfigSync() {
    ...
    var persistentCfg = readBestPersistentConfig();
    if (persistentCfg) {
      cfg = mergeConfig(cfg, persistentCfg);
      cfg._loadedFromPersistentStore = true;
      debug = parseBoolean(cfg.debug, false);
      if (debug) {
        console.log("Location spoofer persistent config -> " + cfg.latitude + "," + cfg.longitude);
      }
    }
  ...
  function loadRuntimeConfig(callback) {
    ...
    function finish() {
      try {
        ...
        if (normalized._loadedFromPersistentStore) {
          persistLastGoodConfig(normalized);
        }
        callback(normalized);
      } catch (err) {
        if (debug) {
        ...
        var fallback = readStoreJson(STORE_KEYS.lastGood);
        if (isStoredConfigCandidate(fallback)) {
          callback(normalizeConfig(mergeConfig(DEFAULT_CONFIG, fallback)));
          return;
        }
  ...
  function continueResponseRewrite(config) {
    ...
    recordRuntimeSuccess(config, responseResult.wifiCount, responseResult.cellCount);
    doneRewriteResponse(responseResult.response, {
      wifiCount: responseResult.wifiCount,
      cellCount: responseResult.cellCount,
      debug: config.debug,
      targetLat: config._effectiveLatitude != null ? config._effectiveLatitude : config.latitude,
      targetLng: config._effectiveLongitude != null ? config._effectiveLongitude : config.longitude
    });
  }
  ...
  var api = {
    ...
    applyStableJitter: applyStableJitter,
    readBestPersistentConfig: readBestPersistentConfig,
